...
There are `r dim(stormData)[1]` records and `r dim(stormData)[2]` variables/attributes in the raw dataset.
...
There are `r length(unique(stormData$EVTYPE))` unique weather event types in the dataset.  However, according to the [Storm Data Documentation](https://d396qusza40orc.cloudfront.net/repdata%2Fpeer2_doc%2Fpd01016005curr.pdf), there should be only 48 offical types. Typos and many inconsistent entries are found in the `EVTYPE` column in the raw dataset. It is therefore cleaned and processed using regular expressions (Regex) and the process is embedded as a middle step in the *processor* function, which is described in more detail in next section.
